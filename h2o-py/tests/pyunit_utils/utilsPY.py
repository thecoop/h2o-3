...
def assertEqualModelMetrics(metrics1, metrics2, tol = 1e-6):
    ...
    for key in metricDict1:
        if (isinstance(metricDict1[key], float)): # only compare floating point metrics
            assert abs(metricDict1[key]-metricDict2[key]) < tol, "ModelMetric {0} from model 1: {1}, from model 2: " \
                                                                 "{2} are different."
